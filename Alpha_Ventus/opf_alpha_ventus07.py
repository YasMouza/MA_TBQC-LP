...
Q1_pu_values = [-0.3, -0.15, 0, 0.15, 0.3]
...
for Q1_pu in Q1_pu_values:
    ...
    model.V = Var(range(n), bounds=(0.95, 1.2))
    model.P = Var(range(n))
    model.Q = Var(range(n))
    ...
    for i in range(n):  
        df = df.append({
            'Q1_pu': Q1_pu,  # Fügen Sie den Wert von Q1_pu zu jedem Eintrag hinzu
            'Busbar': f'Busbar {i+1}',
    ...
    # Fügen Sie das df des aktuellen Durchlaufs zum results_df hinzu
